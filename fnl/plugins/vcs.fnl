(import-macros {: map! : plug! : cmd! : term-cmd!} :macros.vim)

(local {: is-in-arcadia} (require :utils.yndx))

;; Common gitsigns config
(local gitsigns-opts {:signs {:add {:text "▎"}
                              :change {:text "▎"}
                              :delete {:text ""}
                              :topdelete {:text ""}
                              :changedelete {:text "▎"}
                      ...
                      :on_attach (fn [bufnr]
                                   ...
                                   (map! n :<leader>vD #(gs.diffthis "~")
                                         {:desc "Diff This ~"
                                          ...
                                          :buffer true})
                                   (map! ox :ih
                                         (cmd! ":<C-U>Gitsigns select_hunk")
                                         {:desc "GitSigns Select Hunk"
                                          :silent true
                                          :buffer true})
                                   ;; Git management commands
                                   (map! n :<leader>vms
                                         (term-cmd! "git add . && git commit -m $(timestamp) && git push")
...
[(if (is-in-arcadia)
     ...
     {:dir "~/arcadia/contrib/tier1/gitsigns.arc.nvim"
      ...
      :init (fn []
              ...
              (map! n :<leader>fv fzf-arcadia { :desc "Find changed files" }))
      :opts gitsigns-opts}
     ;; Plain gitsigns
     {1 :lewis6991/gitsigns.nvim
      :init (fn [] (map! n :<leader>fv (cmd! "FzfLua git_status") { :desc "Find changed files" }))
      :opts gitsigns-opts})]
