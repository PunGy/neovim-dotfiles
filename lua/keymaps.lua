...
local _local_2_ = require("utils.navigation")
local diagnostic_goto = _local_2_["diagnostic-goto"]
...
vim.keymap.set({"n"}, "<C-b>cn", _3_, {desc = "Copy buffer name"})
local function _4_()
  local cwd_path = vim.fn.fnamemodify(vim.fn.expand("%:p"), ":~:.")
  return vim.fn.setreg("+", cwd_path)
end
vim.keymap.set({"n"}, "<C-b>cp", _4_, {desc = "Copy buffer path"})
...
vim.keymap.set({"n"}, "<leader>dl", _6_, {desc = "Line Diagnostics"})
...
return vim.keymap.set({"n"}, "<Esc>", "<cmd>noh<cr>", {desc = "Clear selection"})
