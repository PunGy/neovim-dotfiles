...
local function diag_jump(count, severity)
  local function _3_()
    local _5_
    do
      local t_4_ = vim.diagnostic.severity
      if (nil ~= t_4_) then
        t_4_ = t_4_[severity]
      else
      end
    ...
    return vim.diagnostic.jump({count = count, severity = _5_})
  end
  return _3_
end
...
vim.keymap.set({"n"}, "<C-x>t", cmd("tabclose"), {desc = "Close tab"})
...
vim.keymap.set({"n"}, "<C-b>cn", _7_, {desc = "Copy buffer name"})
local function _8_()
  local cwd_path = vim.fn.fnamemodify(vim.fn.expand("%:p"), ":~:.")
  return vim.fn.setreg("+", cwd_path)
end
vim.keymap.set({"n"}, "<C-b>cp", _8_, {desc = "Copy buffer path"})
...
vim.keymap.set({"n"}, "<leader>dl", _10_, {desc = "Line Diagnostics"})
...
return vim.keymap.set({"n"}, "<Esc><Esc>", cmd("noh"), {})
